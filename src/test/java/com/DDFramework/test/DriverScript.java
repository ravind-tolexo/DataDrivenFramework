...
public class DriverScript {
    ...
    public static void main(String[] args) throws Exception {
        FileInputStream fs = new FileInputStream("src\\test\\java\\com\\DDFramework\\config\\config.properties");
        CONFIG= new Properties();
        CONFIG.load(fs);

        fs = new FileInputStream("src\\test\\java\\com\\DDFramework\\config\\or.properties");
        OR= new Properties();
        OR.load(fs);
    ...
    public void start() throws IllegalAccessException, IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException{
        ...
        suiteXLS = new Xls_Reader("src\\test\\java\\com\\DDFramework\\xls\\Suite.xlsx");
        ...
        for(currentSuiteID=2;currentSuiteID<=suiteXLS.getRowCount(Constants.TEST_SUITE_SHEET);currentSuiteID++){
            ...
            if(suiteXLS.getCellData(Constants.TEST_SUITE_SHEET, Constants.RUNMODE, currentSuiteID).equals(Constants.RUNMODE_YES)){
                ...
                currentTestSuiteXLS=new Xls_Reader("src\\test\\java\\com\\DDFramework\\xls\\"+currentTestSuite+".xlsx");
